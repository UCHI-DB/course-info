...
GitHub Repo for U. Chicago 23500/33550
...
This document describes what you need to do to get started with git, and also download and upload 23500/33550 labs via GitHub.
...
You should have Git installed and have joined the UCHI-DB organization from
...
1. The first thing we have to do is to clone the current lab repository by
   ...
   ```bash
    $ git clone git@github.com:UCHI-DB/simple-db-hw.git
   ```
...
2. By default the remote called `origin` is set to the location that you cloned
   ...
   ```bash
    $ git remote -v
        origin git@github.com:UCHI-DB/simple-db-hw.git (fetch)
        origin git@github.com:UCHI-DB/simple-db-hw.git (push)
   ```
...
1. All new lab and previous lab solutions will be posted to the
   [labs](https://github.com/UCHI-DB/simple-db-hw) repository in the class
   organization.
